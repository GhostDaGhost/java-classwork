...
public class TwoMethodsA04 {
    ...
    public static void fileAnalysis(String inputFileName, String outputFileName) throws IOException {
		...
		PrintWriter file = new PrintWriter(new FileWriter(outputFileName));

		// INITIALIZE DEFAULT COUNT AND SUM
		int count = 0;
		int sum = 0;

		// INITIALIZE DEFAULT LARGEST AND SMALLEST
		int largest = Integer.MIN_VALUE;
		int smallest = Integer.MAX_VALUE;

		// SCAN FOR KEYBOARD INPUT FOR THE FILE
		while (fileScanner.hasNextInt()) {
			int newNumber = fileScanner.nextInt();
			...
			largest = Math.max(largest, newNumber);
			smallest = Math.min(smallest, newNumber);
		}

		// START WRITING INTO FILE
		if (count > 0) {
			file.println("Numeric Data File \"" + inputFileName + "\" Analysis");
			file.println("Number of integers: " + count);
			file.println("The sum of all integers in file: " + sum);
			file.println("The largest integer in the set: " + largest);
			file.println("The smallest integer in the set: " + smallest);
		} else if (count == 1) {
			file.println("Numeric data file \"" + inputFileName + "\" has only one number: " + sum);
		} else {
			file.println("Numeric data file \"" + inputFileName + "\" is empty");
		}
